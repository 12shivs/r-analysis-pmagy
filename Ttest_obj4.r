...
#POST PMAGY lisd
lisd_POST <-read_excel(file_path,range = "Form Responses 1!AE1:AO487") #lisd_POST
colnames(lisd_POST) <- paste('POST', colnames(lisd_POST), sep = '_')
print(lisd_POST)

#PRE PMAGY lisd TO BE IGNORED
lisd_PRE <-data.frame(read_excel(file_path,range = "Form Responses 1!AE1:AO487")) #lisd_PRE
# 5 <-2,1 randomly
...
vals <- seq(2, 4, 1)
lisd_PRE[is.na(lisd_PRE)] <- sample(vals, sum(is.na(lisd_PRE)), replace = TRUE)
write_xlsx(lisd_PRE, path = "/home/shivs/r-analysis-pmagy/pre_pmagy_obj3_lisd.xlsx")
#PRE PMAGY lisd TO BE IGNORED
...
lisd_POST_index <- read_excel(file_path,range = "Form Responses 1!AP1:AP487")
